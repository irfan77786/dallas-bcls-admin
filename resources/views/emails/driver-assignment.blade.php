    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 6px; background: #fff;">
        <div style="padding: 20px 10px; text-align: center;">
            ...
            <h2 style="margin: 0; font-size: 22px; color: #222;">
                @if(!empty($isAdmin)) Driver assignment notice @else Your driver has been assigned @endif
            </h2>
        ...
        <div style="padding: 10px 16px 24px;">
            @if(!empty($isAdmin))
                <p style="font-size: 14px; margin: 0 0 14px;">A driver was assigned on dispatch for this reservation.</p>
            @else
                <p style="font-size: 14px; margin: 0 0 14px;">
                    Dear {{ $bookingData['passenger_name'] ?? 'Valued Customer' }},
                </p>
                <p style="font-size: 14px; margin: 0 0 14px;">
                    Your chauffeur has been assigned. Details are below.
                </p>
            @endif

            {{-- Driver --}}
            @include('emails.partials.assigned-driver-card', ['driver' => $bookingData['driver'] ?? []])
